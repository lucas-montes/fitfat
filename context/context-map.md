...
| Notifications | [notifications/notifications.md](notifications/notifications.md) | Active-workout rest timer + ongoing foreground notification + planner task reminders (due-time + 30-min pre-reminder, Settings toggle, tap → Plan) + experiment check-in reminders (daily, tap → Plan) |
...
| Experiments | [experiments/experiments.md](experiments/experiments.md) | Experiments as planner items (v24): kind='experiment' rows, PlannerRepository CRUD + check-ins + linked tasks, [Calendar \| Experiments] planner segments, form/detail screens, daily check-in reminder, per-category charts vs 14-day baseline |
