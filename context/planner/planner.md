...
- Task tile: checkbox toggles `done` (strikethrough style when done; fires `Haptics.selection`, T04), tapping the tile opens the **task detail view** (`PlannerItemDetailScreen`, Phase B) with a pencil icon for editing, swipe-to-delete → hard `delete` + `plannerDeleted(title)` top banner with Undo (`commonUndo`) → `restore(PlannerItem)` + invalidate; no confirm dialog (T07); `Haptics.mediumImpact` on delete (T04).
...
- AppBar copy action (in the ⋯ More menu since the appbar-declutter phase): peeks yesterday's pending items via `getByDay(yesterday)`; if none → top banner `plannerCopyNothing` (no rows created); else confirms with the pending count, calls `copyFromPreviousDay(day)`, and shows banner `plannerCopyDone(copied)`.
...
- `lib/src/planner/screens/planner_item_form.dart` — full-screen add/edit route (Phase B): `showPlannerItemDialog(context, {dialogTitle, initialTitle, initialDueDate, initialStartTimeMinutes, initialEndTimeMinutes, initialNotes, initialWorkoutId, initialTags, initialRecurrence})` pushes `PlannerItemFormScreen` and returns a `(String title, DateTime? dueDate, int? startTimeMinutes, int? endTimeMinutes, String? notes, String? workoutId, List<String>? tags, PlannerRecurrence? recurrence)` record or `null` on cancel (close X in the AppBar); inline validation with `plannerTaskRequired`. The AppBar carries the always-visible Save action so saving never requires scrolling. Due-date row: `showDatePicker` (2020–2035), defaults to `initialDueDate`, clear button (`plannerDueDateClear`) → null. Time rows: `showTimePicker`. Tags: chip input with `distinctTags` suggestions. Workout dropdown + recurrence picker mirror the old sheet fields verbatim. The former bottom-sheet wrapper (`planner_item_dialog.dart`) no longer exists.
- `lib/src/planner/screens/planner_item_detail.dart` — read-mostly detail view (Phase B): shows title + done toggle, due date / time range, full note, linked-workout tile (routes by workout status), tags, a recurrence summary, and — with a plain delete confirm — occurrence-safe delete (anchor → whole series, generated occurrence → `deleteOccurrence`, standalone → `delete`). Edit delegates to `PlannerItemFormScreen`; reminders are kept in sync (cancel + re-schedule). Opened from the planner tile tap and the dashboard's upcoming-tasks rows.
...
- **Recurring-task scope (recurring-task-scope, 2026-08-17)** — a task with a repeat rule stores one **anchor** row (`seriesId == id`) that materializes concrete occurrences; `PlannerRecurrence` (daily/weekly/interval/monthly) lives on the anchor and drives `materializeForDay`/`materializeUpTo` (idempotent; `excludedDates` on the anchor prevents regenerating deleted occurrences; `endDate` caps the series). Editing or deleting a **generated occurrence** (not the anchor) prompts a scope dialog (`plannerEditScope*` / `plannerDeleteScope*` / `plannerScopeBody`): "This one only" keeps the per-row behavior; "This and all following ones" — edit applies the non-rule fields (title/notes/times/tags/workout) to the anchor + `updateFutureOccurrences(seriesId, from: day, …)` for existing future rows + re-materializes; delete cancels reminders for the series, `stopSeriesOnOrAfter(seriesId, day)` drops the target + later occurrences and sets the anchor `endDate` to the day before (Undo restores via `restoreSeriesEndDate` then re-materializes). Toggle-done and anchor edits/deletes stay per-instance / whole-series.
