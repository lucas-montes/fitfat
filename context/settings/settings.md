...
`SettingsScreen` (`ConsumerStatefulWidget`) — **hub** (settings-rework, 2026-08-17) of six `ListTile`s
...
- **Budget & Currency** (`_BudgetCurrencyScreen`) — base-currency dropdown, the sync-server card (`_SyncServerCard`: base URL + API key + pull/push actions), and the cached FX-rate list with per-day snapshot rows (see [architecture.md](../architecture.md) and [sync-contract.md](../sync/sync-contract.md)). Each rate row shows `cur → base`, the inverse, the rate date, a "Manual" pill for edit-set rates, plus an edit icon. Transactions in a non-base currency and the recent-transactions rows show the conversion rate applied via `rateUsed` (fx-display T03).
- **Advanced** (`_AdvancedScreen`, 2026-08-24) — six numeric fields promoting formerly-hardcoded tuning constants: planner look-ahead days (`plannerHorizonDays`), calorie-goal adjustment kcal (`calorieGoalAdjustment`), experiment baseline days (`experimentBaselineDays`), default rest between sets in minutes with blank = no default (`defaultRestSeconds`), pre-reminder lead minutes (`reminderLeadMinutes`), and sync request timeout seconds (`apiTimeoutSeconds`). Each commits on submit (keyboard done or check icon); invalid input is rejected by validators and clamped by the setters.
